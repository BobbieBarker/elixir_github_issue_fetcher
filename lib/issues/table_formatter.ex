defmodule Fetcher.TableFormatter do
  import Enum, only: [each: 2, map: 2, map_join: 3, max: 1]

  @doc """
  Takes a list of row data, where each row is a map, and a list of headers.
  Prints a table to STDOUT of the data from each row
  ...
  each column. The `headers` paramater contains the list of columns to extract.
  ## Example
    iex> list = [Enum.into([{"a", "1"}, {"b", "2"}, {"c", "3"}], %{}),
    ...>        Enum.into([{"a", "4"}, {"b", "5"}, {"c", "6"}], %{})]
    iex> Fetcher.TableFormatter.split_into_columns(list, ["a", "b", "c"])
    [["1", "4"], ["2", "5"], ["3", "6"]]
  """
  ...
  ## Examples
    iex> Fetcher.TableFormatter.printable("a")
    "a"
    iex> Fetcher.TableFormatter.printable(99)
    "99"
  """
  ...
  @doc """
  Given a list containing sublists, where each sublist contain the data for a column,
  return a list containing the maximum width of each column.
  ## Example
    iex> data = [["cat", "wombat", "elk"], ["mongooes", "ant", "gnu"]]
    iex> Fetcher.TableFormatter.widths_of(data)
    [6, 8]
  """
  ...
  ## Example
    iex> widths = [5,6,99]
    iex> Fetcher.TableFormatter.format_for(widths)
    "~-5s | ~-6s | ~-99s~n"
  """
  ...
  ## Example
    iex> widths = [5,6,9]
    iex> Fetcher.TableFormatter.seperator(widths)
    "------+--------+----------"
  """
